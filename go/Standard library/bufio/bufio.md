...
NewWriterSize returns a new Writer whose buffer has at least the specified size. If the argument io.Writer is already a Writer with large enough size, it returns the udnerlying Writer.
...
Reset discards any unflushed buffered data, clears any error, and resets b to write its output to w. Calling Reset on the zero value of Writer intializes the internal buffer to the default size. Calling w.Reset(w) (that is , resetting a Writer to itself) does noting.
...
Write writes the contents of p into the buffer. It returns the number of bbytes written. If nn < len(p), it also returns an error explaining why the write is short.
...
WriteRune writes a single Unicode code point, returning the number of bytes written and error.
...
WriteString writes a string. It returns the number of bytes written. If the count is less than les(s), it also returns an error explaining why the write is short.
